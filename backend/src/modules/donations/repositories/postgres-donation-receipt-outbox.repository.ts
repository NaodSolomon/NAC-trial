...
@Injectable()
export class PostgresDonationReceiptOutboxRepository implements DonationReceiptOutboxRepository {
  ...
  async claimBatch(criteria: {
    ...
    const result = await this.pool.query<{
      ...
      `with exhausted as (
         ...
         where type = 'DONATION_RECEIPT_EMAIL'
       ...
       returning outbox.id,
                 outbox.payload ->> 'donationId' as donation_id,
                 outbox.attempts,
                 outbox.lock_token`,
      [
